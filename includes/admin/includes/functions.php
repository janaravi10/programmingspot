...
function query_post_status($post_status){
/*function for querying how many post in certain status*/
global $connection;
$query = "SELECT * FROM posts WHERE post_status = '$post_status'";
$querying = mysqli_query($connection,$query);
return $result_count = mysqli_num_rows($querying);
}
function query_comment_status($comment_status){
	/*function for querying how many comments in certain status in dashboard*/
	global $connection;
	$query = "SELECT * FROM comments WHERE comment_status = '$comment_status'";
	$querying = mysqli_query($connection,$query);
	return	$result_count = mysqli_num_rows($querying);

	}

function query_repeated($table_name){
	//function for querying database repeatedly in index.php file
	global $connection;
	$query = "SELECT * FROM $table_name";
	$querying = mysqli_query($connection,$query);
	return $result_count = mysqli_num_rows($querying);
}
// SUBMIT CATEGORY START
function submit_cat(){
  global $connection;
  if(isset($_POST['add_cat'])){
    $cat_title = $_POST['cat_title'];
    if (!$cat_title) {
      echo "you should provide the info";
    }else{
      $query_insert  = "INSERT INTO categories(cat_title)";
      $query_insert  .= " VALUES('{$cat_title}')";
      $querying  = mysqli_query($connection,$query_insert);
    }

  }
}
// SUBMIT CATEGORY ENDS
// UPDATING CATEGORY START
function update_cat(){
	/* update the category */
  if (isset($_GET['edit'])) {
    if (isset($_POST['update_cat'])) {
      ...
      if (!$update_cat_title) {
        ...
        $query_update  = "UPDATE categories SET cat_title = '{$update_cat_title}'";
        $query_update  .= " WHERE  cat_id = '{$cat_update_id}'";
...
function delete_cat(){
  ...
  if (isset($_GET['delete'])) {
    ...
    $del_querying = mysqli_query($connection,$query);
    header("Location: categories.php");
  }
}

function delete_comment(){
	/* this function will delete the comments */
  global $connection;
...
function users_online(){
	/* function to show how many user availabel online */
  global $connection;
  $session = session_id();
  $time = time();
  $time_out_seconds = 60;
...
function comment_status(){
	/* this will show the comment status */
	global $connection;
...
function check_username_availability(){
	/* if the user want to change his username this function check either the username is aleady exist or not */
global $connection;
...
function del_post()
{
	/* this function is used to delete the post made by certain user */
global $connection;
...
function add_bulk_option(){
	/* this function is used to make the bulk action like deleteing more comments */
	global $connection;
...
function upload_profile_img(){
	global $connection;
	/* this function will upload the profile image of the user */
...
{
	/* this function  will send the message to other user */
	global $connection;
...
{
	/* this function show the messages send and received by the user with specific user */
	global $row_image;
	 global $connection;
...
{
	/* this function will show whether the user already message ? some one */
...
{
	/* this function will indicate the user if he receives any  message */
	global $connection;
	$user_id = $_SESSION['user_id'];
...
{
	/* this to set the message flag as seen when the user click the message sent by anther user */
